...
export default function UpdatePassword() {
    ...
    useEffect(() => {
        // Check if 'Email' session variable exists
        const emailSession = sessionStorage.getItem('Email');
        if (emailSession) {
            // If 'Email' session variable exists, set the email state to its value
            setEmail(JSON.parse(emailSession));
        } else {
            // If 'Email' session variable does not exist, redirect user to login page
            router.push('/login');
        }
    ...
    const handleSubmit = async (e) => {
        ...
        const url = `http://192.168.29.134:1030/updatePassword?${queryParams}`;
    ...
    <div className={styles.home}>
        <div className={styles.left}>
            ...
            <Link href="/student"><button className={styles.button}>Go Back</button></Link>
        </div>
        <div className={styles.right}>
            {/* Form heading */}
            <h1 style={{ color: 'rgba(12, 29, 44, 0.75)' }}>Update Password</h1> 
            <form className={styles.form} onSubmit={handleSubmit}>
                ...
                <div className={styles.alert_success} style={{ display: `${success}` }}>Updated Successfully!</div>
                ...
                <div className={styles.div} style={{ position: 'relative' }}>
                        ...
                        <input name="newPassword" type={showPassword ? 'text' : 'password'} className={styles.input} spellCheck="false" placeholder="Enter new Password" value={newPassword} onChange={handleChange} />
                        <Image
                ...
                <input type="text" name="confirm" value={confirm} onChange={handleChange} className={styles.input} placeholder="Re-enter new Password" style={{ marginTop: '8px' }} />
                <div className={styles.btn_div}>
                <button className={styles.login}>Update</button>
                </div>
            </form>
        </div>
    </div>
